...
- **Blocked** — present when the guide's own documented behavior is affected by an open app bug (see `Zz - Known Bugs/`). A blocked row is invisible on the verification site regardless of its `Verified` value.
