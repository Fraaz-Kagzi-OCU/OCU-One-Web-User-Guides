...
- A record on one of the affected lists (Launcher Apps, Patterns, Availability Types, and likely other Settings lists built the same way).
...
The deactivated record should reappear in the list once **Include closed?** is turned on.

## Actual Result

- Launcher Apps: the list still shows "No results found" — the deactivated "Fleet Tracker" never reappears.
- Patterns: the list still shows "No results found" — the deactivated pattern never reappears.
- Availability Types: the list still shows the same 10 results — "Annual Leave" never reappears.
