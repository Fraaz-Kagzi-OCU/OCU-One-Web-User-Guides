...
On the "Creating an asset" form (and anywhere else the Site picker is used), searching for a site returns "No results found" unless the searching user personally owns that site, is explicitly shared/assigned to it, or the site happens to be marked public. This holds true **even for a user whose role has been explicitly granted "Sites: Read" permission** — that role permission has no effect on what the Site autocomplete returns. In practice this means most users can only attach a site they personally created — any site created by someone else is invisible to them in this picker, regardless of their role's permissions.

## Preconditions

- A test user/role with "Sites: Read" permission explicitly granted.
- A normally-created (private-visibility) site the searching user does not own, isn't shared/assigned to, and isn't public.

## Steps to Reproduce

1. Sign in as a user whose role has explicit "Sites: Read" permission.
2. Open the "Creating an asset" form (or any other screen using the Site picker).
3. Search for a site the user doesn't own, isn't shared/assigned to, and isn't public.

## Expected Result

The site should appear in the results, since the user's role has "Sites: Read" permission.

## Actual Result

"No results found" — the role permission has no effect on what the Site autocomplete returns.
